...
var BitcoinRPC = require('tuxcoind-rpc');
...
Bitcoin.DEFAULT_CONFIG_SETTINGS = {
  ...
  addnode: '192.81.218.35',
  addnode: '159.65.171.116',
  addnode: '159.65.35.43',
  addnode: '174.85.255.102',
  addnode: '159.65.172.127'
};
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './tuxcoin.conf');
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of tuxcoin-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of tuxcoin-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of tuxcoin-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from tuxcoin. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in tuxcoin.conf'
  );
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of tuxcoin-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/tuxcoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/tuxcoin.conf';
    }
  }
  return networkPath;
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/tuxcoind.pid';
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new BitcoinRPC({
        protocol: 'http',
        host: '127.0.0.1',
        port: self.spawn.config.rpcport || 42072,
