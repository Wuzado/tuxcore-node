...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and tuxcore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/tuxcoind')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed tuxcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './tuxcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in tuxcore-node services directory
