...
To develop garlicore-node:

```bash
cd ~
git clone git@github.com:<yourusername>/garlicore-node.git
git clone git@github.com:<yourusername>/garlicore-lib.git
```
...
We now will setup symlinks in `garlicore-node` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf garlicore-lib
ln -s ~/garlicore-lib
...
cd devnode
mkdir node_modules
touch garlicore-node.json
touch package.json
```

Edit `garlicore-node.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../garlicore-node/bin/garlicore-node start
```