...
var Bcoin = require('./gcoin');
var BcoinTx = require('gcoin').tx;
...
P2P.prototype._startBcoin = function(callback) {
  ...
  if (['livenet', 'live', 'main', 'mainnet'].indexOf(this.node.network) !== -1) {
    network = 'main';
    port = this._configPeers[0].port || 42069;
  } else if (this.node.network !== 'regtest') {
    network = 'testnet';
    port = this._configPeers[0].port || 42075;
  } else {
...
P2P.prototype._startBcoinIfNecessary = function(callback) {
  if (!this._hasPeers()) {
    log.info('Peers not explicitly configured, starting a local gcoin node.');
