...
var consensus = require('gcoin').consensus;
...
HeaderService.prototype.getCurrentDifficulty = function() {
  var target = gcoin.mining.common.getTarget(this._lastHeader.bits);
  return gcoin.mining.common.getDifficulty(target);
};
...
HeaderService.prototype._queueBlock = function(block) {
  ...
  self._blockProcessor.push(block, function(err) {
    ...
    log.debug('Header Service: completed processing block: ' + block.rhash() + ' prev hash: ' + gcoin.util.revHex(block.prevBlock));
...
HeaderService.prototype._detectReorg = function(block) {
  return gcoin.util.revHex(block.prevBlock) !== this._lastHeader.hash;
};
