...
/**
 * Will return the path and default garlicore-node configuration. It will search for the
 * configuration file in the "~/.garlicore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.garlicore');
  var defaultConfigFile = path.resolve(defaultPath, './garlicore-node.json');
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    console.log(`The configuration file at '${defaultConfigFile}' is incompatible with this version of Garlicore.`);

    var now = new Date();
    // garlicore-node.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `garlicore-node.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
  ...
  var defaultConfig = {
    ...
    servicesConfig: {
      'insight-api': {
        cwdRequirePath: 'node_modules/insight-grlc-api'
      },
      'insight-ui': {
        cwdRequirePath: 'node_modules/insight-grlc-ui'
      }
    }
  };
